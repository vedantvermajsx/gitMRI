...
public class ReportController {
    ...
    public ReportController(AnalysisJobRepository jobRepository, ReportRepository reportRepository,
        ...
        return reportRepository.findByJobId(jobId)
                .flatMap(report -> jobRepository.findById(jobId).map(job -> {
                    ReportDTO dto = ReportDTO.builder()
                            .jobId(jobId).repoName(job.getRepoName()).repoUrl(job.getRepoUrl())
                            .healthScore(report.getHealthScore()).totalFiles(report.getTotalFiles())
                            .totalClasses(report.getTotalClasses()).totalMethods(report.getTotalMethods())
                            .totalLines(report.getTotalLines()).avgComplexity(report.getAvgComplexity())
                            .maxComplexity(report.getMaxComplexity()).busFactor(report.getBusFactor())
                            .deadCodeCount(report.getDeadCodeCount()).deadCodeRatio(report.getDeadCodeRatio())
                            .hotspotCount(report.getHotspotCount()).totalCommits(report.getTotalCommits())
    ...
    public ResponseEntity<List<ComplexityDTO>> getComplexity(@PathVariable String jobId) {
        List<ComplexityDTO> dtos = complexityRepo.findByJobIdOrderByCcScoreDesc(jobId)
                .stream().map(m -> ComplexityDTO.builder()
                        .id(m.getId()).filePath(m.getFilePath()).packageName(m.getPackageName())
                        .className(m.getClassName()).methodName(m.getMethodName())
                        .ccScore(m.getCcScore()).riskLevel(ccRisk(m.getCcScore()))
                        .lineCount(m.getLineCount()).nestingDepth(m.getNestingDepth())
                        .startLine(m.getStartLine()).build())
    ...
    public ResponseEntity<List<DeadCodeDTO>> getDeadCode(@PathVariable String jobId,
        ...
        List<DeadCodeDTO> dtos = items.stream().map(d -> DeadCodeDTO.builder()
                .id(d.getId()).itemType(d.getItemType()).name(d.getName())
                .qualifiedName(d.getQualifiedName()).filePath(d.getFilePath())
                .lineNumber(d.getLineNumber()).riskLevel(d.getRiskLevel()).reason(d.getReason())
        ...
        List<HotspotDTO> dtos = hotspotRepo.findByJobIdOrderByHotspotScoreDesc(jobId)
                .stream().map(h -> {
                    ...
                    return HotspotDTO.builder()
                            .id(h.getId()).filePath(h.getFilePath()).fileName(fileName)
                            .commitCount(h.getCommitCount()).avgComplexity(h.getAvgComplexity())
                            .hotspotScore(h.getHotspotScore())
                            .riskLevel(score > 0.7 ? "HIGH" : score > 0.4 ? "MEDIUM" : "LOW")
                            .lastModified(h.getLastModified()).build();
                }).collect(Collectors.toList());
        return ResponseEntity.ok(dtos);
    }
    ...
    public ResponseEntity<List<ContributorDTO>> getContributors(@PathVariable String jobId) {
        ...
        List<ContributorDTO> dtos = contributors.stream().map(c -> ContributorDTO.builder()
                .id(c.getId()).authorName(c.getAuthorName()).authorEmail(c.getAuthorEmail())
                .commitCount(c.getCommitCount()).filesOwned(c.getFilesOwned())
                .linesAdded(c.getLinesAdded()).linesRemoved(c.getLinesRemoved())
                .commitPercentage(total > 0 ? Math.round((double) c.getCommitCount() / total * 1000) / 10.0 : 0)
                .firstCommit(c.getFirstCommit()).lastCommit(c.getLastCommit())
                .build()).collect(Collectors.toList());
        return ResponseEntity.ok(dtos);
    }
    ...
    public ResponseEntity<DependencyGraphDTO> getDependencies(@PathVariable String jobId) {
        ...
        List<DependencyGraphDTO.DepNodeDTO> nodeDtos = nodes.stream()
                .map(n -> DependencyGraphDTO.DepNodeDTO.builder()
                        .id(n.getId()).label(n.getArtifactId()).groupId(n.getGroupId())
                        .artifactId(n.getArtifactId()).version(n.getVersion())
                        .scope(n.getScope()).nodeType(n.getNodeType()).build())
                .collect(Collectors.toList());
        List<DependencyGraphDTO.DepEdgeDTO> edgeDtos = edges.stream()
                .map(e -> DependencyGraphDTO.DepEdgeDTO.builder()
                        .id(e.getId()).sourceId(e.getSourceId()).targetId(e.getTargetId()).build())
                .collect(Collectors.toList());
        return ResponseEntity.ok(DependencyGraphDTO.builder().nodes(nodeDtos).edges(edgeDtos).build());
    }
